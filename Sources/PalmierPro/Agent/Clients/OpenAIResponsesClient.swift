...
struct OpenAIResponsesClient: AgentClient {
    ...
    private func run(
        ...
        guard let configuration = runtimeProfile.profile.agent,
              configuration.wireProtocol == .openAIResponses else {
            throw AIProviderError.invalidConfiguration(
                "This provider is not configured for OpenAI Responses."
            )
        }
        ...
        urlRequest.httpBody = try JSONSerialization.data(
            withJSONObject: OpenAIResponsesRequestBody.build(request: request),
            options: [.sortedKeys]
        )
